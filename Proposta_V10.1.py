...
from sklearn.metrics import accuracy_score, classification_report, confusion_matrix, precision_score, recall_score, \
    f1_score, roc_auc_score, log_loss, precision_recall_curve, roc_curve, auc
...
hate_speech_statements = train_data[train_data['polarity'] < -hate_speech_threshold][
...
# Exibir todas as fake news
print("Exibindo todas as fake news:")
all_fake_news = test_data[test_data['label'] == 'pants-fire']['statement'].tolist()
# for i, statement in enumerate(all_fake_news, 1):
#print(f"Fake news {i}: {statement}")

# Aplicar análise de sentimento nas declarações de fake news
print("Realizando análise de sentimento nas fake news...")
fake_news_sentiments = []
for statement in all_fake_news:
    polarity = TextBlob(statement).sentiment.polarity
    subjectivity = TextBlob(statement).sentiment.subjectivity
    fake_news_sentiments.append((statement, polarity, subjectivity))

# Exibir declarações de fake news com polaridade negativa e subjectividade
negative_polarity_threshold = -0.5  # Defina o limiar de polaridade negativa conforme necessário
print(f"\nFake news com polaridade < {negative_polarity_threshold} e subjectividade:")
for statement, polarity, subjectivity in fake_news_sentiments:
    if polarity < negative_polarity_threshold:
        print(f"Sentença: {statement}\nPolaridade: {polarity}\nSubjectividade: {subjectivity}\n")

# Comparação entre fake news e discursos de ódio
print("Verificando interseção entre fake news e discursos de ódio...")
fake_news_set = set(all_fake_news)
hate_speech_set = set(hate_speech_statements['statement'])
intersection = fake_news_set.intersection(hate_speech_set)
if intersection:
    print(f"Sentenças comuns entre fake news e discursos de ódio:")
    for sentence in intersection:
        print(f"Sentença: {sentence}")
else:
    print("Não há sentenças comuns entre fake news e discursos de ódio.")

print(f"Número de sentenças de fake news encontradas no conjunto de discurso de ódio: {len(intersection)}")

# Exibir classificadores utilizados pelo AutoML
print("\nClassificador utilizado pelo AutoML:")
print(auto_ml_model.show_models())

# Exibir arquitetura do modelo MAML
print("\nArquitetura do modelo MAML:")
print(maml)
